...
STANZA_NODE1="${BACKUP_STANZA_NODE1:-pg-mmf-node1}"
STANZA_NODE2="${BACKUP_STANZA_NODE2:-pg-mmf-node2}"
STANZA_NODE3="${BACKUP_STANZA_NODE3:-pg-mmf-node3}"
...
resolve_backup_node() {
    ...
    case "$target" in
        node1|pg-node1|1)  echo "mmf-pg-node1|$STANZA_NODE1" ;;
        node2|pg-node2|2)  echo "mmf-pg-node2|$STANZA_NODE2" ;;
        node3|pg-node3|3)  echo "mmf-pg-node3|$STANZA_NODE3" ;;
...
# ---------------------------------------------------------------------------
cmd_test() {
    log_head "=== Multi-Master Flyway Cluster Integration Tests ==="
    echo ""
    local PASS=0
    local FAIL=0
    ...
    if PGPASSWORD="$PG_PASS" psql -h localhost -p "${HAPROXY_WRITE_PORT:-5532}" -U "$PG_USER" -d "$PG_DB" -tAc "SELECT 1;" >/dev/null 2>&1; then
    ...
    if docker exec mmf-valkey-master valkey-cli -a "${VALKEY_PASSWORD:-changeme_valkey_2025}" --no-auth-warning ping 2>/dev/null | grep -q PONG; then
    ...
    # --- Test 8-10: pgBackRest stanza exists per node ---
    for i in "${!NODES[@]}"; do
        local node="${NODES[$i]}"
