...
STANZA_NODE1="${BACKUP_STANZA_NODE1:-pg-mmp-node1}"
STANZA_NODE2="${BACKUP_STANZA_NODE2:-pg-mmp-node2}"
STANZA_NODE3="${BACKUP_STANZA_NODE3:-pg-mmp-node3}"
...
resolve_backup_node() {
    ...
    case "$target" in
        node1|pg-node1|1)  echo "mmp-pg-node1|$STANZA_NODE1" ;;
        node2|pg-node2|2)  echo "mmp-pg-node2|$STANZA_NODE2" ;;
        node3|pg-node3|3)  echo "mmp-pg-node3|$STANZA_NODE3" ;;
...
# ---------------------------------------------------------------------------
cmd_test() {
    log_head "=== Multi-Master Cluster Integration Tests (pglogical) ==="
    echo ""
    local PASS=0
    local FAIL=0
    ...
    local hap_ok=true
    # Write endpoint
    if PGPASSWORD="$PG_PASS" PGCONNECT_TIMEOUT=3 psql -h localhost -p "${HAPROXY_WRITE_PORT:-5632}" -U "$PG_USER" -d "$PG_DB" -tAc "SELECT 1;" >/dev/null 2>&1; then
        :
    else
        fail "HAProxy write endpoint (port ${HAPROXY_WRITE_PORT:-5632}) not responding"
        hap_ok=false
    fi
    # Read endpoint
    if PGPASSWORD="$PG_PASS" PGCONNECT_TIMEOUT=3 psql -h localhost -p "${HAPROXY_READ_PORT:-5633}" -U "$PG_USER" -d "$PG_DB" -tAc "SELECT 1;" >/dev/null 2>&1; then
        :
    else
        fail "HAProxy read endpoint (port ${HAPROXY_READ_PORT:-5633}) not responding"
        hap_ok=false
    fi
    if $hap_ok; then
        pass "HAProxy write (${HAPROXY_WRITE_PORT:-5632}) and read (${HAPROXY_READ_PORT:-5633}) endpoints responding"
    fi

    # --- Test 6: Valkey connectivity ---
    run_test 6 "Valkey connectivity"
    if docker exec mmp-valkey-master valkey-cli -a "${VALKEY_PASSWORD:-changeme_valkey_2025}" --no-auth-warning ping 2>/dev/null | grep -q PONG; then
