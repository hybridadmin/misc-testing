#!/bin/bash
# =============================================================================
# pglogical Multi-Master Replication Setup Script
# Creates pglogical nodes and bidirectional subscriptions for full-mesh
#
# pglogical key concepts:
#   - Each node is identified by pglogical.create_node()
#   - Subscriptions connect to peer nodes via pglogical.create_subscription()
#   - forward_origins = '{}' prevents replication loops (like origin=none)
#   - pglogical.conflict_resolution controls conflict behavior
#   - pglogical.replicate_ddl_command() allows DDL to replicate
#
...
# pglogical needs superuser or replication role for the provider connection
run_sql_quiet "$DB" "ALTER ROLE $SU SUPERUSER;"

# ---------------------------------------------------------------------------
# Step 3: Configure conflict resolution
# pglogical supports: error, apply_remote, keep_local, last_update_wins, first_update_wins
# last_update_wins uses track_commit_timestamp to compare — real LWW!
# This is a MAJOR advantage over native logical replication.
# ---------------------------------------------------------------------------
...
#   - synchronize_structure = false (we don't want DDL to auto-sync on create)
#   - synchronize_data = false (clean start, no bulk copy)
#   - forward_origins = '{}' (CRITICAL: prevents replication loops)
#     This is pglogical's equivalent of native origin=none
# ---------------------------------------------------------------------------
for peer in "${PEERS[@]}"; do
    PEER_PGL_NAME="${peer//-/_}"
    # Include BOTH subscriber and provider in the name to avoid slot collisions.
    # Without this, node2 and node3 both creating "sub_pg_node1" would collide
    # on the replication slot name on node1.
    ...
    if [ "$EXISTING_SUB" != "1" ]; then
        # Wait for peer to have pglogical extension and node created
