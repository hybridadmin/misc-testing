...
# Then EXCLUDE flyway_schema_history so each node tracks migrations independently.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Exclude flyway_schema_history from the publication
# This table tracks which migrations have been applied on each node.
# If we let it replicate, Flyway on node2 would see node1's migration
# history rows and skip the actual DDL execution — but DDL never replicates,
# so node2 would end up with the history but NOT the schema changes.
#
# By excluding it, each node maintains its own migration history and Flyway
# runs DDL on each node independently.
#
# NOTE: This is safe to run even if flyway_schema_history doesn't exist yet.
# The ALTER PUBLICATION DROP TABLE will simply be a no-op / error that we ignore.
# We also run this on every startup to handle the case where Flyway has
# created the table since last startup.
# ---------------------------------------------------------------------------
FW_TABLE_EXISTS=$(run_sql "$DB" "SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'flyway_schema_history';")
if [ "$FW_TABLE_EXISTS" = "1" ]; then
    log "Excluding flyway_schema_history from publication $PUB_NAME"
    run_sql_quiet "$DB" "ALTER PUBLICATION $PUB_NAME DROP TABLE IF EXISTS flyway_schema_history;"
    log "flyway_schema_history excluded from publication"
else
    log "flyway_schema_history does not exist yet (will be excluded after first Flyway run)"
fi
