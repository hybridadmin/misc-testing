...
├── 15-secrets-store-csi.sh      # (optional) Secrets Store CSI Driver
...
│   └── secrets-store-csi-values.yaml      # (generated by 15)
...
| `SECRETS_STORE_CSI_VERSION` | `1.5.6` | No | Secrets Store CSI Driver Helm chart version |
...
### Step 16 (optional): Secrets Store CSI Driver (CP only)

```bash
# On cp1 ONLY
sudo -E ./15-secrets-store-csi.sh
```

Installs the base [Secrets Store CSI Driver](https://secrets-store-csi-driver.sigs.k8s.io/)
without any provider. This allows pods to mount secrets from external secret
managers (Vault, AWS Secrets Manager, Azure Key Vault, GCP Secret Manager)
as volumes or sync them to Kubernetes Secrets.

After running this script, install the provider for your backend:

| Backend | Provider repo |
|---------|---------------|
| HashiCorp Vault | [vault-csi-provider](https://github.com/hashicorp/vault-csi-provider) |
| AWS Secrets Manager | [secrets-store-csi-driver-provider-aws](https://github.com/aws/secrets-store-csi-driver-provider-aws) |
| Azure Key Vault | [secrets-store-csi-driver-provider-azure](https://github.com/Azure/secrets-store-csi-driver-provider-azure) |
| GCP Secret Manager | [secrets-store-csi-driver-provider-gcp](https://github.com/GoogleCloudPlatform/secrets-store-csi-driver-provider-gcp) |
...
| 15* | `15-secrets-store-csi.sh` | CP only | ~1 min |
| | | **Total** | **~30 min** |

*\* Optional step -- not required for a functional cluster.*
