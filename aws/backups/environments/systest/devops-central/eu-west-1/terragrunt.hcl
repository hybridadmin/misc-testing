# Systest DevOps Central - eu-west-1 (Ireland)
...
inputs = {
  ...
  # NOTE: backup_account_id is not defined for systest. When a backup account is provisioned,
  # update the backup_account_id in env.hcl and this ARN will resolve correctly.
  route53_backup_role_arn = local.env_vars.locals.backup_account_id != "" ? "arn:aws:iam::${local.env_vars.locals.backup_account_id}:role/ADMIN-SYSTEST-BACKUP-CrossAccountBackupRole" : ""
