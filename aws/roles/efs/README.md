...
| `aws_efs_file_system` | General-purpose encrypted EFS filesystem |
| `aws_efs_backup_policy` | Automatic backup enabled |
...
├── _envcommon/
│   └── efs.hcl                       # Shared component config (module source, env inputs)
├── modules/
│   └── efs/
...
| `service` | `string` | Service identifier (always `efs`) |
...
| `service` | `string` | `"efs"` | Service identifier |
...
```bash
cd envs/prodire/eu-west-1/111111111111/efs
terragrunt plan
terragrunt apply
```
...
Key:    efs/<region>/terraform.tfstate
...
Key:    efs/eu-west-1/terraform.tfstate
```
...
3. Create region/component directories:
   ```bash
   mkdir -p envs/<new-env>/<region>/efs
   ```
...
2. Create the region directory:
   ```bash
   mkdir -p envs/<env>/<new-region>/efs
   ```
...
| `AWS::EFS::FileSystem` (with inline `BackupPolicy`) | `aws_efs_file_system.this` + `aws_efs_backup_policy.this` |
...
| `LogRetentionDays` parameter | Removed -- not applicable (no CloudWatch logs in EFS module) |
