...
locals {
  ...
  #   envs/<environment>/<region>/bastion/terragrunt.hcl              (single account)
  #   envs/<environment>/<region>/<account_id>/bastion/terragrunt.hcl (multi-account)
  ...
  # For single-account envs (systest), account_id comes from env.hcl.
  # For multi-account envs (prodire), each account dir has an account.hcl with its own account_id.
...
remote_state {
  ...
  config = {
    ...
    key            = "${local.service}/${path_relative_to_include()}/terraform.tfstate"
